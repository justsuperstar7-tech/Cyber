...
BOT_TOKEN = "8991345070:AAHvacR-NTVfiFLPkoEh1nZo0yMA2MX1hAQ"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "mydeals":
        ...
        await query.edit_message_text("📋 <b>Your Deals</b>\n\nSelect an option:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        return
    ...
    if query.data == "mypending":
        ...
        await query.edit_message_text("⏳ <b>You have no Pending deals!</b>", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        return
    ...
    if query.data == "new_deal":
        ...
        await query.edit_message_text("💰 <b>Create New Deal</b>\n\nAdmin will verify and process it.", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        return
    ...
    if query.data == "create_deal":
        ...
        keyboard = [[premium_button("Back", "🔙", "back", "onyx")]]
        await query.edit_message_text(
            f"✅ <b>Deal Created Successfully!</b>\n\n📊 <b>Deal ID:</b> #{deal_id}\n💰 <b>Amount:</b> 10.5 TON | 250 USDT | 1500 INR\n⏳ <b>Status:</b> Pending",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="HTML"
        )
    ...
    if query.data == "deal_categories":
        ...
        keyboard = [[premium_button("Back", "🔙", "back", "onyx")]]
        await query.edit_message_text(f"📂 <b>Deal Categories</b>\n\n{cat_text}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        return
    ...
    if query.data == "payment_methods":
        ...
        keyboard = [[premium_button("Back", "🔙", "back", "onyx")]]
        await query.edit_message_text(f"💳 <b>Payment Methods</b>\n\n{method_text}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        return
    
    # History
    if query.data == "history":
        user_transactions = [t for t in data["transactions"] if t.get("user") == str(user_id)]
        if user_transactions:
            history_text = "\n".join([f"• {t.get('type', 'Deal')}: {t.get('amount', 'N/A')} - {t.get('status', 'N/A')}" for t in user_transactions[-5:]])
            text = f"📈 <b>Your Recent History</b>\n\n{history_text}"
        else:
    ...
    if query.data == "leaderboard":
        ...
        if top_users:
            lb_text = "🏆 <b>Top Users Leaderboard</b>\n\n"
            for i, (uid, stats) in enumerate(top_users, 1):
                user_name_display = data["users"].get(uid, {}).get("name", uid)
                deals = stats.get("deals_count", 0)
                medals = {1: "🥇", 2: "🥈", 3: "🥉"}
                medal = medals.get(i, f"#{i}")
                lb_text += f"{medal} <b>{escape_html(user_name_display)}</b>\n📊 Deals: {deals}\n\n"
        else:
            lb_text = "🏆 <b>No users yet! Be the first!</b>"
        keyboard = [[premium_button("Back", "
