...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "globalstats":
        ...
        keyboard = [
            [premium_button("Refresh", "🔄", "globalstats")],
            [premium_button("Back", "🔙", "back")]
        ]
        await query.edit_message_text(
            f"🌍 Global Stats\n\n"
    ...
    if query.data == "toggle_notifications":
        ...
        save_data(data)
        await query.edit_message_text(
            f"✅ Notifications {'Enabled' if stats['notifications'] else 'Disabled'}!",
            parse_mode="HTML"
        )
        return
    ...
    if query.data == "add_default_deal":
        ...
        keyboard = [[premium_button("Back", "🔙", "admin_panel")]]
        await query.edit_message_text(
            f"✅ Deal Added! Total: {stats['total_deals']}",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="HTML"
        )
        return
    ...
    if query.data == "admin_edit_stats":
        ...
        keyboard = [
            ...
            [premium_button("USDT", "📊", "edit_usdt")],
            [premium_button("INR", "📊", "edit_inr")],
            [premium_button("Deals", "📊", "edit_deals")],
            [premium_button("Back", "🔙", "admin_panel")]
        ]
        await query.edit_message_text(
    ...
    if query.data == "admin_reset":
        if not is_admin(user_id):
            return
        keyboard = [
            [InlineKeyboardButton("⚠️ YES", callback_data="admin_reset_confirm")],
            [premium_button("Cancel", "❌", "admin_panel")]
        ]
    ...
    if query.data == "admin_giveaway":
        if not is_admin(user_id):
            return
        keyboard = [
            [premium_button("Start", "🎁", "giveaway_start")],
            [premium_button("Pick Winner", "🏆", "giveaway_pick_winner")],
            [premium_button("Back", "🔙", "admin_panel")]
        ]
